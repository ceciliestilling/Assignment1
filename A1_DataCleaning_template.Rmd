...
```{r}
# Changing the name of col names so they are the same
...
```{r}
LU_train[2] <- str_extract(LU_train$VISIT,'\\d')
token_train[2] <- str_extract(token_train$VISIT,'\\d')
...
```{r}
train_2 <- merge(demo_train_subset, LU_train_subset)

train <- merge(train_2, token_train_subset)
...
```{r}
df <- train[train$VISIT == "1",]


df2 <- select(df, SUBJ, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)
colnames(df2)[2] <- "ADOS1"
colnames(df2)[3] <- "MullenRaw1"
colnames(df2)[4] <- "ExpressiveLangRaw1"
colnames(df2)[5] <- "Socialization1"

merge_train <- merge(df2, train)
```
...
#giving understandable values to gender 
merge_train$Gender[merge_train$Gender == "1"] <- "F"
merge_train$Gender[merge_train$Gender == "2"] <- "M"

#chaning the names of diagnosis
merge_train$Diagnosis <- as.character(merge_train$Diagnosis)

merge_train$Diagnosis[merge_train$Diagnosis == "A"] <- "ASD"
merge_train$Diagnosis[merge_train$Diagnosis == "B"] <- "TD"

#anonymizing the participants
merge_train$SUBJ <- as.factor(merge_train$SUBJ)
a <- seq(1, 61)
levels(merge_train$SUBJ) <- a

write.csv(merge_train, file = 'train.csv')
```
